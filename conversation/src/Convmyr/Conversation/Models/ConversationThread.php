...
namespace Convmyr\Conversation\Models;
...
class ConversationThread extends Eloquent
{
    use SoftDeletes;
    ...
    {
        $this->table = ConversationModels::table('threads');
    ...
    {
        return $this->hasMany(ConversationModels::classname(ConversationMessage::class), 'thread_id', 'id');
    }
    ...
    {
        return $this->hasMany(ConversationModels::classname(ConversationParticipant::class), 'thread_id', 'id');
    }
    ...
    {
        $participantsTable = ConversationModels::table('participants');
        $threadsTable = ConversationModels::table('threads');
    ...
    {
        $participantTable = ConversationModels::table('participants');
        $threadsTable = ConversationModels::table('threads');
    ...
    public function addParticipants(array $participants)
    {
        if (count($participants)) {
            foreach ($participants as $user_id) {
                ConversationModels::participant()->firstOrCreate([
                    'user_id' => $user_id,
                    'thread_id' => $this->id,
                ]);
            }
    ...
    public function participantsString($userId = null, $columns = ['first_name', 'last_name'])
    {
        ...
        //$userNames = $participantNames->lists($usersTable . '.first_name' . ' ' . $usersTable . '.last_name');
